...
parser.add_argument(
    ...
    "--reverse",
    action="store_true",
    help="Downloads from oldest videos to the newest.",
)
parser.add_argument(
    "-d", "--duration", action="store_true", help="Adds duration entry to output file."
)
...
with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # SPECIAL YT-DLP OBJECT WITH YDL_OPTS (OPTIONS)
    try:
        result = ydl.extract_info(
            args.url, download=False
        )  # WE WANT INFO WITHOUT DOWNLOADING THE VIDEO
    except yt_dlp.utils.DownloadError:  # SILLY YOU, THAT IS NOT A VALID
        sys.exit()

    file_name = f"{result['uploader'].replace(' ', '_')}.txt"
    with open(
        file_name, "w+t", encoding="utf-8"
    ) as file:  # CREATE A NEW FILE AND MAKE SURE HE IS CLEAN
        file.seek(0)
        file.truncate()
        ...
        for i, item in enumerate(playlist):  # LOOPS ENTRIES TO GRAB EACH VIDEO URL
            ...
            if args.duration:
                duration = single_video[
                    "duration"
                ]  # DURATION IS AN INT TYPE IN THE SECONDS
                ...
                with open(
                    file_name, "a+t", encoding="utf-8"
                ) as file:  # OPEN FILE AND APPEND
